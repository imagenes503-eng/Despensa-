// ===== GOOGLE SHEETS INTEGRATION — SYNC BIDIRECCIONAL =====
// Exporta hacia Sheets (POST) e importa desde Sheets (GET) en cualquier teléfono.

let _autoSyncTimer = null;
const AUTO_SYNC_MS = 5 * 60 * 1000; // 5 minutos

// ── Envío POST (escribe en Sheets) ───────────────────────────────────
async function sheetsEnviar(accion, datos) {
  const url = localStorage.getItem("vpos_sheetsURL") || "";
  if (!url) return;
  try {
    await fetch(url, {
...
// ── Importar TODO desde Sheets (GET) ─────────────────────────────────
async function sheetsImportar() {
  const url = localStorage.getItem("vpos_sheetsURL") || "";
  if (!url) { toast("Primero configura Google Sheets ⚙️", true); return; }

  toast("⏳ Importando desde Sheets…");
  try {
    const resp = await fetch(url + "?accion=EXPORTAR");
    ...
    if (data.productos && data.productos.length) {
      productos = data.productos.map(r => ({
        id:     Number(r['ID'])             || Date.now(),
        nom:    String(r['Nombre']          || ''),
        cat:    String(r['Categoría']       || ''),
        compra: parseFloat(r['Precio Compra']) || 0,
        venta:  parseFloat(r['Precio Venta'])  || 0,
        stock:  parseInt(r['Stock'])           || 0,
        min:    parseInt(r['Stock Mínimo'])    || 0
      }));
      cambios++;
    }

    // ── Ventas diarias ───────────────────────────────────────────────
    if (data.ventasDiarias && data.ventasDiarias.length) {
      // Fusionar: no sobrescribir entradas locales más nuevas
      const mapaSheets = {};
      data.ventasDiarias.forEach(r => {
        const fecha = r['Fecha'] ? String(r['Fecha']).substring(0, 10) : null;
        if (fecha) mapaSheets[fecha] = {
          fecha,
          monto: parseFloat(r['Monto']) || 0,
          nota:  String(r['Nota'] || '')
        };
      });
      // Añadir/sobreescribir entradas remotas que no estén en local
      const mapaLocal = {};
      ventasDiarias.forEach(v => { mapaLocal[v.fecha] = v; });
      Object.keys(mapaSheets).forEach(f => {
        if (!mapaLocal[f]) mapaLocal[f] = mapaSheets[f]; // solo agrega, no borra locales
      });
      ventasDiarias = Object.values(mapaLocal).sort((a, b) => a.fecha.localeCompare(b.fecha));
      cambios++;
    }

    // ── Historial de ventas (solo leer, no sobreescribir historial local) ──
    if (data.ventas && data.ventas.length) {
      ...
      data.ventas.forEach(r => {
        ...
        if (id && !idsLocales.has(id)) {
          // Reconstruir objeto mínimo para el historial
          historial.push({
            id,
            fecha:  r['Fecha']      || '',
    ...
    if (cambios) {
      ...
      actualizarTodo();
      const ts = data.ts ? new Date(data.ts).toLocaleTimeString('es', {hour:'2-digit', minute:'2-digit'}) : '';
      toast(`✅ Sincronizado desde Sheets${ts ? ' · ' + ts : ''}`);
    } else {
      toast("✓ Sin cambios nuevos en Sheets");
    }
    ...
    console.error("[Sheets import]", e);
    toast("⚠️ No se pudo importar: " + e.message, true);
  }
}
...
function iniciarAutoSync() {
  ...
  if (activo) {
    ...
    console.log("[Sheets] Auto-sync cada 5 min activado");
  }
}

// ── Config modal ──────────────────────────────────────────────────────
function abrirConfigSheets() {
  document.getElementById("sheetUrlInput").value = localStorage.getItem("vpos_sheetsURL") || "";
...
async function sheetsExportarProductos() {
  const url = localStorage.getItem("vpos_sheetsURL") || "";
  if (!url) { toast("Primero configura Google Sheets ⚙️", true); return; }
  toast("Exportando inventario a Sheets…");
  const filas = productos.map(p => [p.id, p.nom||"", p.cat||"", p.compra||0, p.venta||0, p.stock||0, p.min||0]);
  await sheetsEnviar("PRODUCTOS", { filas });
  toast("✓ Inventario enviado a Google Sheets");
}
...
async function sheetsExportarVentasDiarias() {
  ...
  toast("✓ Ventas diarias enviadas a Google Sheets");
}
...
async function sheetsExportarTodo() {
  const url = localStorage.getItem("vpos_sheetsURL") || "";
  if (!url) { toast("Primero configura Google Sheets ⚙️", true); return; }
  toast("⏳ Exportando todo a Sheets…");
  const filasP = productos.map(p => [p.id, p.nom||"", p.cat||"", p.compra||0, p.venta||0, p.stock||0, p.min||0]);
  const filasVD = ventasDiarias.map(v => [v.fecha, v.monto, v.nota || ""]);
  await sheetsEnviar("PRODUCTOS", { filas: filasP });
  await sheetsEnviar("VENTAS_DIARIAS", { filas: filasVD });
  localStorage.setItem("vpos_ultimoSync", new Date().toISOString());
  _actualizarBadgeSync();
  toast("✅ Todo exportado a Google Sheets");
}
// ===== FIN SHEETS =====
